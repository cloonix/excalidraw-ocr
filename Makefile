...
help:
	...
	@echo "  make run          - Run with docker-compose (shows help)"
...
shell:
	...
	docker-compose run --rm ocr /bin/bash
...
test:
	...
	docker-compose run --rm ocr node --version
	@echo "\n$(BLUE)Testing cairosvg import:$(NC)"
	docker-compose run --rm ocr python -c "import cairosvg; print('✓ cairosvg OK')"
	@echo "\n$(BLUE)Testing lz-string:$(NC)"
	docker-compose run --rm ocr node -e "const lz = require('lz-string'); console.log('✓ lz-string OK')"
	@echo "\n$(GREEN)✓ All tests passed$(NC)"

list-models:
	@echo "$(GREEN)Listing available OCR models...$(NC)"
	docker-compose run --rm ocr python ocr.py --list-models
...
ocr:
	...
	@if [ -n "$(OUTPUT)" ]; then \
		docker-compose run --rm ocr python ocr.py $(IMAGE) -o $(OUTPUT); \
	else \
		docker-compose run --rm ocr python ocr.py $(IMAGE); \
	fi
...
excalidraw:
	...
	docker-compose run --rm ocr python excalidraw_ocr.py $(FILE)
...
batch-ocr:
	...
	@for img in input/*.{png,jpg,jpeg,PNG,JPG,JPEG}; do \
		if [ -f "$$img" ]; then \
			echo "Processing $$img..."; \
			docker-compose run --rm ocr python ocr.py "/$$img" -o "/output/$$(basename $$img .png).txt" 2>/dev/null || \
			docker-compose run --rm ocr python ocr.py "/$$img" -o "/output/$$(basename $$img .jpg).txt" 2>/dev/null || \
			docker-compose run --rm ocr python ocr.py "/$$img" -o "/output/$$(basename $$img .jpeg).txt" 2>/dev/null; \
		fi; \
	done
...
batch-excalidraw:
	...
	docker-compose run --rm ocr python excalidraw_ocr.py /input/
	@echo "$(GREEN)✓ Batch processing complete$(NC)"
